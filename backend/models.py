...
class Usuario(AbstractBaseUser, PermissionsMixin):
    ...
    region = models.CharField('Región', max_length=50, blank=True)
    comuna = models.CharField('Comuna', max_length=50, blank=True)
    ...
    imagen = models.ImageField(
        'Imagen de perfil', upload_to='perfil', blank=True, null=True)
    is_active = models.BooleanField(default=True)
    is_staff = models.BooleanField(default=False)
    objects = UsuarioManager()

    USERNAME_FIELD = 'username'
    REQUIRED_FIELDS = ['email', 'nombre', 'apellido', 'direccion', 'region', 'comuna', 'telefono']
...
class Producto(models.Model):
    ...
    imagen = models.ImageField(
        'Imagen del producto', upload_to='productos/', blank=True, null=True)
...
class Pedido(models.Model):
    ...
    estado = models.CharField('Estado', max_length=20,
                              choices=ESTADO_CHOICES, default='en_carrito')
...
class ItemPedido(models.Model):
    ...
    producto = models.ForeignKey(
        'Producto', on_delete=models.PROTECT, related_name='item_pedidos')
    precio = models.DecimalField('Precio', max_digits=10, decimal_places=2)
    ...
    def clean(self):
        ...
        if self.cantidad > self.producto.stock:
            raise ValidationError(
                f'No hay suficiente stock para {self.producto.nombre}. Disponible: {self.producto.stock}, solicitado: {self.cantidad}.')
    ...
    def save(self, *args, **kwargs):
        ...
        # Recargar el estado del producto para reflejar cambios recientes en el stock
...
class ItemCarrito(models.Model):
    ...
    producto = models.ForeignKey(
        'Producto', on_delete=models.PROTECT, related_name='item_carritos')
    precio = models.DecimalField('Precio', max_digits=10, decimal_places=2)
    cantidad = models.IntegerField('Cantidad', default=1)
    ...
    def __str__(self):
        return f'{self.cantidad} x {self.producto.nombre} - {self.carrito}'
